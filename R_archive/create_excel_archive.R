...
create_excel_archive <- function(l, filename = "data_archive",
  ...
  if(!is.null(convention_table_path)){
    convention_table <- read_excel_tmp(path = convention_table_path)
  ...
  if(!is.null(column_headers_path)){
    ch <- read_excel_tmp(path = column_headers_path) %>% filter(keep)
    paste2 <- function(x, sep = "") {paste(x[!is.na(x)], collapse = sep)}

    ### Here: at the moment this is done with `join_by(table, variable)`.
    ### BUT: some variables were shifted to a new table
    ###      Their description is currently lost!
    ###      Maybe better: join_by(variable, orig_name)?
    readme <- readme %>%
      rows_patch(ch %>% select(table, variable, unit, description, axis_legend, orig_name),
                 by = c("table", "variable"), unmatched = "ignore") %>%
      left_join(ch %>% select(table, variable, additional_description),
                by = join_by(table, variable)) %>%
      ...
      ungroup() %>%
      # mutate(additional_description = if_else(!is.na(additional_description), paste0("; Additional: ", additional_description), ""),
      #        description = paste0(description, additional_description)) %>%
...
# 7) Optional feature: Add Code for dbdiagram.io
